...
export async function getLocationTypeFields(locationTypeId) {
  ...
  const res = await fetch(`${API_BASE}/location-types/${locationTypeId}/fields`, { headers })
  return handleResponse(res, 'fetch location type fields')
}

export async function createLocationTypeField(locationTypeId, payload) {
  const headers = await authHeaders()
  const res = await fetch(`${API_BASE}/location-types/${locationTypeId}/fields`, {
    method: 'POST',
    headers,
    body: JSON.stringify(payload),
  })
  return handleResponse(res, 'create location type field')
}

export async function updateLocationTypeField(fieldId, payload) {
  const headers = await authHeaders()
  const res = await fetch(`${API_BASE}/location-type-fields/${fieldId}`, {
    method: 'PATCH',
    headers,
    body: JSON.stringify(payload),
  })
  return handleResponse(res, 'update location type field')
}
...
export async function deleteLocationTypeField(fieldId) {
  ...
  return handleResponse(res, 'delete location type field')
}
