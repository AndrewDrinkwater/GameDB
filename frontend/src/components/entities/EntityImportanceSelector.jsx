...
import { updateEntityImportance } from '../../api/entities.js'
import './EntityImportanceSelector.css'
...
export default function EntityImportanceSelector({ entityId, importance, onUpdate }) {
  ...
  const handleSelect = async (newImportance) => {
    ...
    setLoading(true)
    try {
      await updateEntityImportance(entityId, newImportance)
      onUpdate?.(newImportance)
    } catch (err) {
      console.error('Failed to update entity importance', err)
